...
```bash
npx skills add hoangtmbk/skills --skill=setup-hoangta-skills
```

```bash
npx skills update setup-hoangta-skills
```

[Source](https://github.com/hoangtmbk/skills/tree/main/skills/engineering/setup-hoangta-skills)

## What it does

`setup-hoangta-skills` teaches one repo how the engineering skills should behave in it — where issues live, what the triage labels are called, and where the domain docs sit — and records those answers as **config** the other skills read.
...
You invoke this by typing `/setup-hoangta-skills` — the agent won't reach for it on its own.
...
`setup-hoangta-skills` is a **run-once setup** — the foundation the whole engineering set stands on, not a step you repeat. Its neighbours are the skills that read what it writes: `triage`, because it applies the label vocabulary configured here, and `to-spec` / `to-tickets`, because they publish into the issue tracker configured here. Run it first; everything downstream assumes it has.
