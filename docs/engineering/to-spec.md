...
`to-spec` publishes into your issue tracker, so [setup-hoangta-skills](https://aihero.dev/skills-setup-hoangta-skills) must have configured the tracker and triage labels for this repo first. It applies the `ready-for-agent` label itself — no separate triage pass needed.
