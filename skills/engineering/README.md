...
- **[playwright-qa](./playwright-qa/SKILL.md)** — Headless functional QA of running web apps. Pairs with the `bowser-qa-agent` subagent and the `/browser-qa` + `/qa-clean` slash commands (all ship in this plugin) to fan out YAML user-stories in parallel and produce PASS/FAIL Run reports.
- **[setup-hoangta-skills](./setup-hoangta-skills/SKILL.md)** — Scaffold the per-repo config (issue tracker, triage label vocabulary, domain doc layout) that the other engineering skills consume.
